...
$(document).ready(function(){
	$("#myInput").on("keyup", function() {
  	...
  	$(".add").click(function(){
  		...
  		$('.fourm,.can').removeClass('hide');
  		$('.add,#product_name_update').addClass('hide');
  		// $('.fourm').reset();
  	})
  	$(".can").click(function(){
  		$('.fourm').addClass('hide');
	...
	$.ajax({
		...
		success:function(result){
			...
			if (result != '') {
				...
				"<th>S.No</th><th>Sub Category</th><th>Category</th><th>Status</th><th>Action</th></tr>";
				res = JSON.parse(result);
				// list2.push(res);
				for (var i = 0; i < res.length; i++) {
					debugger;
...
function edit_product(id){
	...
	$.ajax({
		...
		success:function(result){
			...
			$("#insert-product,#product_name_update,#add-product,#can-product,.fourm").removeClass("hide");
			...
			$("#category_list").val(cid);
			sub_category_list(cid);
			$("#sub_category_list").val(sid);
			$("#brand_list").val(bid);
			$("#product_name_submit,.add").addClass("hide");
		}
	})
}
...
function web_setting(){
	...
	obj.submit = 'select';
	$.ajax({
		url:"../controllers/web_setting_controller.php",
		method:"POST",
		data:obj,
		success:function(result){
			debugger;
			var path = "../media/company-logo/";
			var data = JSON.parse(result);
			$("#address").val(data[0].address);
			$("#desc").val(data[0].description);
			$("#email").val(data[0].email);
			$("#facebook_url").val(data[0].facebook_url);
			$("#google_plus_url").val(data[0].google_plus_url);
			$("#likedin_url").val(data[0].likedIn_url);
			$("#telephone").val(data[0].telephone);
			$("#id").val(data[0].id);
			$("#twitter_url").val(data[0].twitter_url);
			$("#youtube_url").val(data[0].youtube_url);
			$("#img1").attr('src',path + data[0].logo);
		}
	})
}
// function myFunction() {
//   var value = $(this).val().toLowerCase();
//     $(".table-hover td").filter(function() {
//       $(this).toggle($(this).text().toLowerCase().indexOf(value) > -1)
//     });
// }
